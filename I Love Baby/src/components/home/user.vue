<template>
  <div class="template-content post-box user-box">
    <div class="filter clearfloat">
      ...
      <div class="select-box clearfloat">
        <div class="title">User ID</div> <input type="text" class="authorText" v-model="userJson.userId" /> </div>
      <div class="select-box clearfloat">
      ...
      <div class="select-box clearfloat">
        ...
        <div class="xiala-box">
          <v-selection :selections="accountStatusList" :selectionactive="userJson.status" @on-change="onStatusChange($event)"></v-selection>
        </div>
      </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      init() {},
      onStatusChange(val) {
        this.userJson.status = val.label;
      },
      refreshUser() {
      ...
      bannedUser(identifier) {
        ...
        bannedModal = new Modal({
          "eleId": "bannedModal",
          "title": "",
          "content": "<div class='input-box'><span>Banned user " + identifier + " </span><input type='number' placeholder='(1~100)' /><span> days</span><div><div class='reason-box'><input type='text' placeholder='Please enter reasons'/><span class='noReasenhint'></span><div>",
          success: function() {
              ...
              if($(".reason-box input").val() != "") {
                ...
                $.ajax({
                  ...
                  success: function(rex) {
                    $("#bannedModal").hide();
                    that.refreshUser();
                    that.addNewsJson.content = "Your account is banned for " + limitadminUserJson.days + " days because of " + limitadminUserJson.reason;
                    that.addNewsJson.userId = limitadminUserJson.userId;
                    let addNewsStr = JSON.stringify(that.addNewsJson);
                    $.ajax({
                ...
                $(".noReasenhint").html("*Please enter reasons");
              }
            } else {
              $(".noReasenhint").html("*Banned天数为1~100");
      ...
      clockedUser(identifier) {
        ...
        clockedModal = new Modal({
          "eleId": "clockedModal",
          "title": "",
          "content": "<div class='reason-box'><input type='text' placeholder='Please enter reasons' class='liangModal'/><span class='noReasenhint'></span></div>",
          success: function() {
            if($(".reason-box input").val() == "") {
              $(".noReasenhint").html("*Please enter reasons");
            } else {
              ...
              $.ajax({
                ...
                success: function(rex) {
                  $("#clockedModal").hide();
                  that.refreshUser();
                  that.addNewsJson.content = "Your account is clocked because of " + limitadminUserJson.reason;
                  that.addNewsJson.userId = limitadminUserJson.userId;
                  let addNewsStr = JSON.stringify(that.addNewsJson);
                  $.ajax({
                    url: that.baseUrl + "addMessageSys",
      ...
      resumeUser(identifier) {
        ...
        resumeModal = new Modal({
          "eleId": "resumeModal",
          "title": "",
          "content": "Are you sure to  restore the normal operation of user " + identifier + "？",
          success: function() {
            let limitadminUserJson = {};
            limitadminUserJson.userId = identifier;
            ...
            $.ajax({
              ...
              success: function(rex) {
                $("#resumeModal").hide();
                that.refreshUser();
                that.addNewsJson.content = "Your account is back to normal";
                that.addNewsJson.userId = limitadminUserJson.userId;
                let addNewsStr = JSON.stringify(that.addNewsJson);
                $.ajax({
                  url: that.baseUrl + "addMessageSys",
