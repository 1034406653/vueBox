<template>
  <div class="template-content  post-box social-box">
    <div class="filter clearfloat">
      <div class="select-box clearfloat">
        <div class="title">Type</div>
        <div class="xiala-box">
          <v-selection :selections="postTypeList" :selectionactive="postType"  @on-change="onSocialTypeChange($event)"></v-selection>
        </div>
      </div>
      <div class="select-box clearfloat">
      ...
      <div class="select-box clearfloat">
        ...
        <div class="xiala-box">
          <v-selection :selections="postStatusList" :selectionactive="postStatus" @on-change="onSocialStatusChange($event)"></v-selection>
        </div>
      </div>
      <div class="select-box clearfloat">
    ...
    <div class="content">
      <table border="0" cellspacing="0" cellpadding="0">
        ...
        <tr v-for="trLi in trList" :key="trLi.communionId">
          ...
          <td>{{trLi.nickname?trLi.nickname:"--"}}</td>
          <td>{{trLi.location?trLi.location:"--"}}</td>
          <td>{{trLi.time?trLi.time:"--"}}</td>
          ...
          <td>{{trLi.pageView?trLi.pageView:"0"}}</td>
          <td>{{trLi.replyNum?trLi.replyNum:"0"}}</td>
          <td>{{trLi.communionId?trLi.communionId:"--"}}</td>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      onSocialTypeChange(val) {
      	console.log(val);
        this.postType = val.label;
      },
      onSocialStatusChange(val) {
      	console.log(val);
        this.postStatus = val.label;
      },
      refresh(option) {
      ...
      resumePost(resumePostId) {
        ...
        resumePostModal = new Modal({
          ...
          "title": "",
          "content": "Are you sure to resume the post " + resumePostId + "？<span class='resumeHint'></span>",
          success: function() {
            $.ajax({
